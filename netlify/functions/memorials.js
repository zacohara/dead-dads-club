...
export default async (req) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const result = await client.query(`SELECT id, father_name, years, message, submitted_by, anniversary_date, created_at FROM ddc_memorials ORDER BY created_at DESC LIMIT 50`);
      await client.end();
    ...
    if (req.method === 'POST') {
      ...
      if (!father_name || !message) { await client.end(); return new Response(JSON.stringify({ error: 'Father name and message required' }), { status: 400, headers }); }
      if (father_name.replace(/[^a-zA-Z]/g, '').length < 2) { await client.end(); return new Response(JSON.stringify({ error: 'Please enter a valid name' }), { status: 400, headers }); }
      // Validate anniversary_date format if provided
      let validDate = null;
      if (anniversary_date) {
        const d = new Date(anniversary_date);
        if (!isNaN(d.getTime())) validDate = anniversary_date;
      }
