...
<div style={{width:28,height:1,background:`${G}50`}}/><span style={{fontFamily:OF,fontSize:12,fontWeight:600,letterSpacing:5,color:G,textTransform:"uppercase"}}>{text}</span>
...
<a href="#memorial-wall" className="hide-mobile" style={{color:"#888",fontSize:13,textDecoration:"none",letterSpacing:2,textTransform:"uppercase"}}>Wall</a>
<a href="#the-box" className="hide-mobile" style={{color:"#888",fontSize:13,textDecoration:"none",letterSpacing:2,textTransform:"uppercase"}}>Box</a>
<a href="#quiz" className="hide-mobile" style={{color:"#888",fontSize:13,textDecoration:"none",letterSpacing:2,textTransform:"uppercase"}}>Quiz</a>
<a href="#top" style={{padding:"10px 24px",border:`1px solid ${G}`,color:G,fontSize:12,textDecoration:"none",letterSpacing:3,textTransform:"uppercase",borderRadius:3,fontWeight:600}}>Join</a>
</div></nav>)}
...
<span style={{fontSize:12,color:G,fontWeight:600,whiteSpace:"nowrap",letterSpacing:1}}>{Math.max(100-(count||0),0)} left</span>
...
<button onClick={go} disabled={sub} style={{padding:"12px 20px",background:G,color:D,border:"none",borderRadius:3,fontFamily:OF,fontSize:12,fontWeight:700,letterSpacing:2,textTransform:"uppercase",cursor:"pointer"}}>
...
<p style={{color:"#888",fontSize:13,marginTop:12}}>Screenshot this. You earned your place.</p></div>)}
...
{rj.length>0&&<div style={{marginTop:20,height:24,overflow:"hidden"}} aria-live="polite"><div key={ti} style={{fontSize:14,color:"#666",animation:"fadeUp 0.5s ease"}}>
<span style={{color:`${GL}99`}}>{rj[ti]?.name}</span> joined · {ago(rj[ti]?.time)}</div></div>}
<p style={{marginTop:14,fontSize:13,color:"#444"}}>Founding Members lock in their rate for life.</p>
</div>
...
<p style={{color:"#666",fontSize:13,marginTop:20,letterSpacing:3,textTransform:"uppercase"}}>— {who}</p></div></div>)}
...
<span style={{fontSize:11,color:G,letterSpacing:4,textTransform:"uppercase",fontWeight:600}}>Welcome Box</span></div>
...
<span style={{fontSize:12,color:G,letterSpacing:3,textTransform:"uppercase",fontWeight:600}}>🔒 Unlock free</span></div></div>
...
<button onClick={go} disabled={sub} style={{padding:"16px 24px",background:G,color:D,border:"none",cursor:"pointer",fontFamily:OF,fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase"}}>
...
<div style={{fontSize:11,color:"#888",letterSpacing:2,textTransform:"uppercase",marginTop:6}}>{u.l}</div></div>))}</div>}
...
<p style={{fontSize:14,color:"#aaa",fontStyle:"italic",marginTop:8,lineHeight:1.6}}>"{lastEntry.message?.substring(0,80)}..."</p></div>
...
style={{flex:1,padding:"16px",background:"#0D0D0D",border:"none",color:CR,fontFamily:OF,fontSize:16,outline:"none"}}/>
<button onClick={fGo} style={{padding:"16px 24px",background:G,color:D,border:"none",cursor:"pointer",fontFamily:OF,fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase"}}>Join</button></div>
<button onClick={()=>setSFunnel(false)} style={{marginTop:14,background:"transparent",border:"none",color:"#555",fontFamily:OF,fontSize:13,cursor:"pointer"}}>Maybe later</button></>):
...
<div style={{marginBottom:14}}><label style={{fontSize:12,color:`${G}99`,letterSpacing:2,textTransform:"uppercase",fontWeight:500,marginBottom:6,display:"block"}}>Date of passing</label>
...
<p style={{fontSize:12,color:"#555",marginTop:4,fontStyle:"italic"}}>We'll remember him with you on this day each year.</p></div>
...
<span style={{position:"absolute",bottom:20,right:10,fontSize:11,color:form.message.length>1800?RD:"#444"}}>{form.message.length}/2000</span></div>
...
{e.years&&<p style={{fontSize:13,color:G,letterSpacing:2,marginBottom:14,fontWeight:500}}>{e.years}</p>}
<p style={{fontSize:15,color:"#bbb",lineHeight:1.7,fontStyle:"italic"}}>"{e.message}"</p>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:16}}>
{e.submitted_by&&<span style={{fontSize:13,color:"#666"}}>— {e.submitted_by}</span>}
<button onClick={()=>shareCard(e)} title="Download shareable card" style={{background:"transparent",border:`1px solid ${DB}`,borderRadius:3,padding:"6px 10px",color:"#666",fontSize:12,cursor:"pointer",fontFamily:OF}}>
...
<div><div style={{fontSize:15,color:CR,fontWeight:600}}>Dead Dads Club</div><div style={{fontSize:12,color:"#4CAF50",fontWeight:500}}>Incoming Call</div></div></div>
...
<a href="#top" style={{padding:"14px 28px",background:G,color:D,textDecoration:"none",fontFamily:OF,fontSize:13,fontWeight:700,letterSpacing:2,textTransform:"uppercase",borderRadius:3}}>Join</a>
<button onClick={()=>{setLi(-1);setP(false);setTimeout(start,100)}} style={{padding:"14px 20px",background:"transparent",border:`1px solid ${DB}`,color:"#888",borderRadius:3,fontFamily:OF,fontSize:13,cursor:"pointer"}}>↻ Replay</button></div>)}</div>)}</div></SW>)}
...
style={{flex:1,padding:"16px",background:"#0D0D0D",border:"none",color:CR,fontFamily:OF,fontSize:16,outline:"none"}}/>
<button onClick={es} style={{padding:"16px 24px",background:G,color:D,border:"none",cursor:"pointer",fontFamily:OF,fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase"}}>Join</button></div>
...
<button onClick={()=>{setS(0);setA([]);setSent(false);setE("");setErr("");setMyNum(null)}} style={{marginTop:20,padding:"10px 20px",background:"transparent",border:`1px solid ${DB}`,color:"#666",borderRadius:3,fontFamily:OF,fontSize:13,cursor:"pointer"}}>↻ Retake</button>
</div>)}</div></SW>)}
...
{t.tag&&<div style={{position:"absolute",top:16,right:-28,background:G,color:D,fontSize:10,fontWeight:700,padding:"4px 32px",transform:"rotate(45deg)",letterSpacing:2}}>{t.tag}</div>}
...
<button onClick={()=>setShow(false)} style={{position:"absolute",top:10,right:14,background:"transparent",border:"none",color:"#555",fontSize:20,cursor:"pointer"}}>×</button>
...
<input type="email" placeholder="Your email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()}
style={{flex:1,padding:"16px",background:"#0D0D0D",border:"none",color:CR,fontFamily:OF,fontSize:16,outline:"none"}}/>
<button onClick={go} style={{padding:"16px 24px",background:G,color:D,border:"none",cursor:"pointer",fontFamily:OF,fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase"}}>Join</button></div>):
...
style={{flex:1,padding:"16px",background:"#0D0D0D",border:"none",color:CR,fontFamily:OF,fontSize:16,outline:"none"}}/>
<button onClick={()=>load(key)} style={{padding:"16px 24px",background:G,color:D,border:"none",cursor:"pointer",fontFamily:OF,fontSize:13,fontWeight:700,letterSpacing:2}}>Go</button>
</div></div></div>);
...
<div style={{fontFamily:CG,fontSize:36,fontWeight:700,color:G}}>{s.n}</div><div style={{fontSize:13,color:"#aaa",marginTop:6}}>{s.l}</div></div>))}</div>
...
<th style={{padding:"14px 16px",textAlign:"left",color:G,fontSize:13,letterSpacing:2,fontWeight:600}}>Email</th>
<th style={{padding:"14px 16px",textAlign:"left",color:G,fontSize:13,letterSpacing:2,fontWeight:600}}>Joined</th>
</tr></thead>
...
<span style={{fontSize:12,color:"#666"}}>{new Date(m.created_at).toLocaleDateString()}</span></div>
{m.years&&<p style={{fontSize:13,color:G,marginBottom:6}}>{m.years}</p>}
{m.anniversary_date&&<p style={{fontSize:13,color:"#4CAF50",marginBottom:6}}>📅 Anniversary: {m.anniversary_date}</p>}
<p style={{fontSize:15,color:"#bbb",fontStyle:"italic",lineHeight:1.6}}>"{m.message}"</p>
<p style={{fontSize:13,color:"#666",marginTop:8}}>— {m.submitted_by}</p></div>))}</div></div>)}
